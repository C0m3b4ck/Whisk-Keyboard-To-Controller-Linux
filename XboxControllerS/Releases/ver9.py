...
def check_hotkey_state():
    """Check if Ctrl+Alt+M are pressed at the same time. Returns True or False."""
    data = disp.query_keymap()  # Use Display, not root

    # Keysyms for Control_L, Alt_L, and 'm'
    keysyms = {
        "Control_L": 0xffe3,
        "Alt_L": 0xffe9,
        "m": 0x006d  # 'm' lowercase
    }
    ...
    try:
        ctrl_down = is_key_down(keycodes[0])
        alt_down = is_key_down(keycodes[1])
        m_down = is_key_down(keycodes[2])
        return ctrl_down and alt_down and m_down
    except Exception:
        return False
...
print("Holding Ctrl+Alt+M disables mouse cursor center warp.")
