...
- Button X: X
- Button Y: Y
- Left trigger: Middle mouse button
- Right trigger: Left mouse button
- BTN_MODE (special): Right mouse button
- D-pad: Arrow keys
- Show this help: H
    """
    print(kb)
...
def hotkey_check(event_key, pressed):
    """Detect hotkeys M, H, Emergency, Shift+Alt+P toggling inside keyboard_event loop."""
    global current_sensitivity, current_sensitivity_index

    # For edge detection and state management we store static vars on function attributes
    ...
    # Detect on keydown of any involved key, then confirm all pressed
    emergency_keys = {'KEY_LEFTSHIFT', 'KEY_RIGHTSHIFT', 'KEY_X', 'KEY_Q', 'KEY_S'}
    # We'll trigger when event_key is one of these keys pressed
...
def keyboard_thread():
    ...
    for event in keyboard.read_loop():
        ...
        if event.type == evdev.ecodes.EV_KEY:
            ...
            # Note KEY_Q here is used for left stick click and emergency hotkey,
            # those can coexist since emergency triggers only on all pressed.
            if key == 'KEY_Q':
                ...
                # Also note 'S' is part of the emergency combo
                if key == 'KEY_S':
