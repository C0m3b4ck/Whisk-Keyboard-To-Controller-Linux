...
# Detect keyboard by checking if device has EV_KEY with typical keyboard keys (A-Z)
def find_keyboard():
    for dev in devices:
...
# Detect mouse by checking if device has EV_REL with REL_X/Y and EV_KEY with BTN_LEFT
def find_mouse():
    for dev in devices:
        caps = dev.capabilities()
