...
class MovingAverage:
    def __init__(self, size=10):  # Increased window size for smoother mouse
        self.size = size
    ...
    def average(self):
        if not self.xs or not self.ys:
            return 0, 0
        return sum(self.xs) / len(self.xs), sum(self.ys) / len(self.ys)  # float avg

smoother = MovingAverage()

# Cursor centering toggle management
cursor_centering_enabled = True  
cursor_centering_lock = threading.Lock()

def check_hotkey_pressed():
    """Check if Shift+Alt+P are pressed simultaneously."""
    data = disp.query_keymap()
    keysyms = {
        "Shift_L": 0xffe1,
        "Alt_L": 0xffe9,
        "P": 0x0070,  # 'p' lowercase
    }
...
def keyboard_thread():
    ...
    for event in keyboard.read_loop():
        ...
        # Handle cursor centering toggle on Shift+Alt+P press (edge detection)
        hotkey_pressed_now = check_hotkey_pressed()
        if hotkey_pressed_now and not was_hotkey_pressed:
        ...
        was_hotkey_pressed = hotkey_pressed_now
