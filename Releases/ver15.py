...
class MovingAverage:
    def __init__(self, size=20):  # increased size for smoother averaging
        self.size = size
...
cursor_centering_enabled.set()  # initially enabled

cursor_locked = threading.Event()  # state for cursor grab (locked)
...
def ungrab_cursor():
    disp.ungrab_pointer(X.CurrentTime)  # Fix: ungrab on Display, not Window
    disp.sync()
    print("Cursor unlocked (ungrabbed).")
    cursor_locked.clear()

def check_hotkey_pressed(keys):
    """ Check if all keys in 'keys' list currently pressed simultaneously via X query_keymap """
    data = disp.query_keymap()
    keycodes = [disp.keysym_to_keycode(k) for k in keys]
...
def hotkey_polling_thread():
    # Hotkey Shift+Alt+P to toggle cursor centering
    was_pressed = False

    # Emergency hotkey Ctrl+Shift+Alt+X to disable cursor lock + centering and print notice, works all the time
    emergency_was_pressed = False

    while True:
        # Check Shift + Alt + P
        shift_alt_p = check_hotkey_pressed([0xffe1, 0xffe9, 0x0070])  # Shift_L, Alt_L, 'P'

        if shift_alt_p and not was_pressed:
        ...
        was_pressed = shift_alt_p

        # Check emergency hotkey Ctrl + Shift + Alt + X
        ctrl_shift_alt_x = check_hotkey_pressed([0xffe3, 0xffe1, 0xffe9, 0x0078])  # Ctrl_L, Shift_L, Alt_L, 'X'

        if ctrl_shift_alt_x and not emergency_was_pressed:
            # Emergency switch-off: disable cursor centering and unlock if locked
            if cursor_locked.is_set():
                ungrab_cursor()
            if cursor_centering_enabled.is_set():
                cursor_centering_enabled.clear()
            print("Emergency switch-off activated: cursor centering disabled and cursor unlocked!")
        emergency_was_pressed = ctrl_shift_alt_x
...
def keyboard_thread():
    ...
    for event in keyboard.read_loop():
        if event.type == evdev.ecodes.EV_KEY:
            ...
            # Handle cursor lock toggle on 'N' key press (edge detection)
            if key == 'KEY_N':
...
def mouse_thread():
    ...
    for event in mouse.read_loop():
        if event.type == evdev.ecodes.EV_REL:
            ...
            # Do NOT clear smoother here — keep sliding window for smoothness
...
print("Press Ctrl+Shift+Alt+X for EMERGENCY cursor unlock and centering OFF.")
