...
def hotkey_polling_thread():
    ...
    while not exiting.is_set():
        ...
        if check_hotkey_pressed([SHIFT_L, X, Q, S]):
        ...
        if check_hotkey_pressed([H]):
            if not was_pressed_show_help:
                print_keybinds()
            was_pressed_show_help = True
        else:
            was_pressed_show_help = False
...
threads.append(threading.Thread(target=hotkey_polling_thread, daemon=True))
